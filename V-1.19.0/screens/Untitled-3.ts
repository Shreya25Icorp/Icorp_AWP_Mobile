...
const UserHome = () => {
  ...
  const handleClockIn = async (shiftId: any, isUpcoming: boolean) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${SERVER_URL_ROASTERING}/clockin/clockout/mobile/${shiftId}/${userId}`,
  ...
  const handleClockOut = async (shiftId: any, isUpcoming: any) => {
    if (isUpcoming) setLoadingUpcoming(true);
    else setLoadingFollowing(true);
  ...
  const checkLocation = (position: any) => {
    ...
    const distance = getDistance(
      { latitude, longitude },
      STATIC_LOCATION // Replace with the actual location for the shifts
    );
    ...
    // If the user is at the location
    if (distance < locationMeters) {
      setIsAtLocation(true);
  
      // Upcoming shift
      if (!wasAtLocation && upcomingShift) {
        const isAttendanceEmpty = !upcomingShift?.attendance || Object.keys(upcomingShift.attendance).length === 0;
        const hasStartTime = upcomingShift?.attendance?.startTime;
  
        // Auto clock-in if user isn't clocked in and hasn't started the shift yet
        if (!isClockedIn && isAttendanceEmpty) {
          console.log("Auto clocking in for upcoming shift...");
          handleClockInOut(upcomingShift?._id, true); // Clock in
          setIsClockedIn(true);
        }
        setWasAtLocation(true); // Mark that user is at the location
      }
  
      // Following shift
      if (!wasFollowingAtLocation && followingShift) {
        const isFollowingAttendanceEmpty = !followingShift?.attendance || Object.keys(followingShift.attendance).length === 0;
        const hasFollowingStartTime = followingShift?.attendance?.startTime;
  
        if (!isFollowingClockedIn && isFollowingAttendanceEmpty) {
          console.log("Auto clocking in for following shift...");
          handleClockInOut(followingShift?._id, true); // Clock in for following shift
          setIsFollowingClockedIn(true);
        }
        setWasFollowingAtLocation(true); // Mark that user is at the location for the following shift
      }
      ...
      // Auto clock-out for upcoming shift if user leaves location
      if (wasAtLocation && upcomingShift) {
        const hasEndTime = upcomingShift?.attendance?.endTime;
  
        if (!hasEndTime) {
          console.log("Auto clocking out for upcoming shift...");
          handleClockInOut(upcomingShift?._id, false); // Clock out
          setIsClockedIn(false);
        }
        setWasAtLocation(false); // Reset the location state
      }
  
      // Auto clock-out for following shift if user leaves location
      if (wasFollowingAtLocation && followingShift) {
        const hasFollowingEndTime = followingShift?.attendance?.endTime;
  
        if (!hasFollowingEndTime) {
          console.log("Auto clocking out for following shift...");
          handleClockInOut(followingShift?._id, false); // Clock out
          setIsFollowingClockedIn(false);
        }
        setWasFollowingAtLocation(false); // Reset the location state
      }
    }
  };
  ...
  const onRefresh = useCallback(() => {
    ...
    // Run both functions concurrently and wait for both to finish
    Promise.all([checkPermissionStatus(), fetchData()])
      .then(() => {
        // Once both functions have completed, stop the refreshing indicator
        setIsRefreshing(false);
      })
      .catch((error) => {
        // Handle any errors that might occur in either function
        console.error('Error during refresh:', error);
        setIsRefreshing(false); // Ensure the refreshing indicator is stopped in case of an error
      });
  ...
  const checkShiftStatus = (shift: any, min: number) => {
    ...
    if (isNaN(startShiftTime.getTime()) || isNaN(endShiftTime.getTime())) {
      console.error('Invalid shift times:', shift.shiftStartDateTime, shift.shiftEndDateTime);
      return false;
    }

    const fifteenMinBefore = new Date(startShiftTime.getTime() - min * 60 * 1000);
    const convertedNow = convertLocalToUTCISO(now);

    // Check if current time is within valid clock-in range
    return (
      (convertedNow >= fifteenMinBefore.toISOString() || convertedNow > startShiftTime.toISOString()) &&
  ...
  const fetchAndCheckShiftStatus = async () => {
    try {
      const response = await axios.get(SERVER_URL_ROASTERING + '/get/button', { withCredentials: true });
      const mobileMin = response.data.button.find(item => item.useFor === 'mobile');
      ...
      if (mobileMin) {
        ...
        if (followingShift) {
          setIsFollowingEnabled(checkShiftStatus(followingShift, mobileMin.range));
        }
      }
    } catch (error) {
