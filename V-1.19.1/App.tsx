...
import PositionDuties from "./screens/PositionDuties";
...
import SitePINScreen from "./screens/SitePINScreen";
import PreviewImages from "./screens/PreviewImages";
import ImageViewer from "./components/CustomeImageViewer/CustomImageViewer";
import PushNotification from "react-native-push-notification";
import PushNotificationIOS from '@react-native-community/push-notification-ios';
import messaging from '@react-native-firebase/messaging';
import AtmosphericReport from "./screens/atmosphericReport";
...
function App(): JSX.Element {
  ...
  //     const scanData = await AsyncStorage.getItem('scanData');
  //     if (scanData) {
  //       const { scanInterval, extraScanOptionId, messageToDisplay } = JSON.parse(scanData);
  //       const currentTime = Date.now();

  //       if (currentTime >= scanInterval) {
  //         console.log('Triggering modal globally...');
  //         setModalVisible(true);

  //         // Set modal state based on variables
  ...
  //         // Update the next trigger time for the interval
  //         const scanInterval = JSON.parse(scanData).scanInterval;
  //         const updatedTriggerTime = currentTime + scanInterval;
  //         await AsyncStorage.setItem('scanData', JSON.stringify({ ...JSON.parse(scanData), nextTriggerTime: updatedTriggerTime }));
  //       }
  //     }
  //   };

  //   const intervalId = setInterval(checkInterval, 1000); // Check every second
  ...
  useEffect(() => {
    const fetchVersionsAndCompare = async () => {
      try {
        ...
        const appStoreUrl =
          "https://apps.apple.com/in/app/active-workforce-pro/id6670550788";
          ...
          Alert.alert(
            ...
            [
              ...
              },
              {
                ...
                style: "cancel", // Adds a cancel-style button
              },
            ],
            { cancelable: true } // Enable dismissing the alert by tapping outside
          );
        } else {
  ...
  //     },
  //     popInitialNotification: true, // Pop the notification if the app was launched by it
  //     requestPermissions: true, // Request permissions when the app is opened
  //   });
  // }, []);

  // useEffect(() => {
  //   // Configure PushNotification for both iOS and Android
  //   PushNotification.configure({
  //     onRegister: function (token) {
  //       console.log('TOKEN:', token);
  //     },
  ...
  //       // Process the notification for both platforms
  //       notification.finish(PushNotificationIOS.FetchResult.NoData);
  //     },
  //     popInitialNotification: true, // Automatically pops the initial notification on app startup
  //     requestPermissions: Platform.OS === 'ios', // Request permission for iOS only
  //   });

  //   // Request FCM permission for both iOS and Android
  //   messaging()
  //     .requestPermission()
  //     .then((authStatus) => {
  //       if (
  ...
  //       }
  //     })
  //     .catch((error: any) => console.error('Permission error:', error));

  //   // Additional iOS-specific configuration (if needed)
  //   if (Platform.OS === 'ios') {
        ...
        <GestureHandlerRootView style={{ flex: 1 }}>
          <View style={{ flex: 1 }}>
            ...
            <NavigationContainer linking={linking}>
              {isLoading ? (
                ...
                <Stack.Navigator>
                  ...
                  />
                  <Stack.Screen
                    name="AtmosphericReport"
                    component={AtmosphericReport}
                    options={{ headerShown: false }}
                  />
                  <Stack.Screen
                    name="Documents"
            ...
            <CustomNFCModal
              ...
              onActivityLogPress={() => console.log('Activity Log')}
              onIncidentReportPress={() => console.log('Incident Report')}
              onMaintenanceIssuePress={() => console.log('Maintenance Issue')}
