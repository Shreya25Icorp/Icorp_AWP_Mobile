...
const PositionDuties = () => {
  ...
  const renderItem = ({ item }: { item: Checkpoint }) => (
    <TouchableOpacity key={item._id} onPress={() => handleNfcDiscovery()}>
        ...
        <View style={styles.mainLoc}>
          ...
          <View style={styles.locationText}>
            ...
            <View style={styles.locContainer}>
              ...
              />
              <Text style={styles.NumberGuardsText}>{item.location.name}</Text>
            </View>
            <View style={styles.locContainer}>
              <Icon
  ...
  const handleNfcDiscovery = async () => {
    try {
      ...
      // const response = await axios.get(
      //   `${SERVER_URL_ROASTERING}/nfc/checkpoint/${tagId}`,
      //   {
      //     withCredentials: true,
      //   }
      // );
      const response = await axios.get(
        `${SERVER_URL_ROASTERING}/nfc/checkpoint/04B30B12C17484`,
        {
          withCredentials: true,
        }
      );
      ...
      if (responseData?.success && responseData?.checkpoint) {
        ...
        const isPositionIdMatch = CheckPoint.checkpointId?.positionId?.some(
          (position) => position._id === shift.positionId?._id
        );
