...
const SubmitReports = () => {
  ...
  // console.log("reportType and id:", reportType, id, status);
    ...
    try {
      ...
      while (capturing) {
        ...
        if (image && image.path) {
          ...
          setImageUris((prevUris) => [...prevUris, image.path]);
        }
        capturing = false; // Set to true to continue capturing
      }
    } catch (error) {
  ...
  const openGallery = async () => {
    try {
      ...
      const combinedUris = [...imageUris, ...newUris];
  ...
  const fetchActivityCategory = useCallback(async () => {
    try {
      ...
      const url = `${SERVER_URL_ROASTERING}/get/check/types?report=siteActivityLog&siteId=${shift.siteId._id}`;
      ...
      if (response?.status === 200) {
        ...
        setActivityCategories(transformedCategories); // Bind to state
      } else {
        console.error(
          "API request failed:",
          response?.status,
  ...
  // const fetchActivityType = useCallback(
  //   async (ActivityCategoryId: string) => {
  //     try {
  //       setIsLoading(true);
  ...
  //         };
  //         const payload = {
  //           activityTypeId: [ActivityCategoryId],
  //         };
  //         const url = `${SERVER_URL_ROASTERING}/get/sub/activity/type/activity/type/wise`;
  ...
  //           const data = response?.data?.subActivityTypes || [];
  ...
  //               severityLevel: item.severityLevel,
  //             }));
  //             setActivityTypes(transformedCategories);
  //           }
  //         } else {
  //           console.error(
  //             'API request failed:',
    ...
    useCallback(() => {
      const fetchData = async () => {
        try {
          setIsLoading(true);
          if (status === "draft") {
            const url = `${SERVER_URL_ROASTERING}/get/site/activity/log/${id}`;
            ...
            if (response?.status === 200) {
              ...
              const siteActivityLog = response?.data?.siteActivityLog;
              const reportedAt = siteActivityLog?.reportedAt;
              const activityType = siteActivityLog?.activityType;
              const severityLevel = siteActivityLog?.severityLevel;
              const location = siteActivityLog?.location;
              const subLocation = siteActivityLog?.subLocation;
              const otherLocation = siteActivityLog?.otherLocation;
              const Details = siteActivityLog?.comment;
              const outCome = siteActivityLog?.outCome;
  ...
  const fetchLocation = useCallback(async () => {
    try {
      ...
      const siteId = await AsyncStorage.getItem("siteId");
      const limit = 10;
      let allLocations: { label: string; value: string; subLocations?: any[] }[] = [];
      ...
      if (initialResponse?.status !== 200) {
        console.error("API request failed:", initialResponse?.status, initialResponse?.statusText);
        return;
      }
      ...
      if (allLocations.length > 0 && !allLocations.some((loc) => loc.value === "other")) {
      ...
      const selectedLoc = allLocations.find((loc) => loc.value === selectedValueLocation);
      ...
      //   // ✅ Auto-select subLocation if it's in the list
      //   if (selectedValueSubLocation) {
      //     const isSubLocationValid = formattedSubLocations.some((sub) => sub.value === selectedValueSubLocation);
  ...
  const handleChangeActivityCategory = async (
    ...
    if (value !== "") {
      ...
      setActivityCategoryError(value.trim() === "");
      setSeverityLevel(severityLevel);
  ...
  const handleSubmit = async (isDraft: boolean) => {
    try {
      ...
      if (selectedValue.trim() === "") {
        setActivityCategoryError(true);
        isValid = false;
      } else {
        setActivityCategoryError(false);
      }
      ...
      if (isValid
        ...
        const apiUrl =
          status === "draft"
            ? `${SERVER_URL_ROASTERING}/update/user/activity/log/${id}`
            : `${SERVER_URL_ROASTERING}/create/user/activity/log`;
        ...
        if (combinedDateTime) {
          ...
          formattedReportedDate = new Date(
            ...
            .toISOString()
            .replace("Z", "912Z"); // Replace the 'Z' with '912Z'
        ...
        formData.append("activityType", selectedValue);
        // if (selectedValueType !== '') {
        //   formData.append('subActivityType', selectedValueType);
        // }
  ...
  const handleSaveDraft = async (isDraft: boolean) => {
    try {
      ...
      if (selectedValue.trim() === "") setActivityCategoryError(true);
      if (severityLevel.trim() === "") setPriorityError(true);
      ...
      if (
        ...
        setIsLoading(true);
        const apiUrl = `${SERVER_URL_ROASTERING}/create/user/activity/log`;
      ...
      <SafeAreaView style={styles.container}>
            ...
            <View style={globalStyles.whiteBox}>
              <View style={styles.textContainer}>
                ...
                <View style={styles.titleContainer}>
                  <CustomText style={styles.titleText}>Activity Log</CustomText>
                </View>
              </View>
              ...
              {isLoading ? (
                ...
                <View style={styles.formContainer}>
                  ...
                  <View style={styles.inputGroup}>
                    ...
                    <CustomBottomSheet
                      ...
                      labelBottomsheet="Activity Category"
                      items={activityCategories}
                      searchText={"Search for Activity Category..."}
                      isRequired={true}
                      onValueChange={(value) => {
                        const selectedItem = activityCategories.find(
                    ...
                    {activityCategoryError && (
                      <HelperText type="error" visible={activityCategoryError}>
                ...
                <View style={styles.inputGroup}>
                  <CustomBottomSheet
                      ...
                      labelBottomsheet="Activity Type"
                      items={activityTypes}
                      searchText={'Search for Activity Type...'}
                      isRequired={true}
                      onValueChange={value => {
                        const selectedItem = activityTypes.find(
                          item => item.value === value,
                        );
                        if (selectedItem) {
                  ...
                  {/* <View style={styles.inputGroup}>
                    <TextInput
                      mode="outlined"
                      outlineColor={description ? '#2E9E4A' : '#BFBBBB'}
                      activeOutlineColor="#2E9E4A"
                      style={[
                        styles.textAreaDesc,
                        {
        ...
        <>
          <TouchableOpacity
            ...
            <Image
              source={require('../assests/manager/overlayProfile.png')}
          ...
          <View style={styles.bottomOverlayImageContainer}>
            <Image
              source={require('../assests/images/homebottom.png')}
